# Version: 1.0.13 (最終修正版: インデント修正、favicon.ico回避、RTタイムアウト延長)
# Last Major Update: インデントエラーと favicon.ico 処理を追加。RTタイムアウトを15秒に延長。

from flask import Flask, render_template, request, abort # abortを追加
from google.transit import gtfs_realtime_pb2
from datetime import datetime, timezone, timedelta
import urllib.request, urllib.error # urllib.errorを明示的にインポート
import pandas as pd
import numpy as np
import io
...
APP_PORT = 5000
TIMEOUT_SECONDS = 15 # GTFS-RTのタイムアウト時間を15秒に設定
...
def get_realtime_updates():
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # ★修正箇所: タイムアウト時間を延長 (TIMEOUT_SECONDS)
            ...
            # ★修正箇所: タイムアウトエラーの場合、詳細な理由を出力
            reason = str(e.reason) if hasattr(e, 'reason') else "不明"
            print(f"GTFS-RT取得エラー (Attempt {attempt+1}): [URL/CONNECTION ERROR] 理由: {reason}")
            slp(1)
...
def favicon():
    # faviconのリクエストがメインロジックに流れるのを防ぐために、404を返して処理を終了させる
    return abort(404)
